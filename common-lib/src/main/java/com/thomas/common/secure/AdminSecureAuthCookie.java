...
public class AdminSecureAuthCookie {

    static final Logger logger = LoggerFactory.getLogger(AdminSecureAuthCookie.class);

    private static int AUTHCOOKIE_LENGTH = 43;
    private static String md5Key = "auth_teaq%^#$";
    private static String aesAuthKey = "aes_twad*@%!";
    private static String md5Symbol = "md5s2";
    private static String aesSymbol = "adss2";
    ...
    public static AdminSecureAuthCookie decodeAuthCookie(String sAuthcookie) {
        try {
            ...
            if (decrypted == null) {
                ...
                AdminSecureAuthCookie secureAuthCookie = valueOf(decrypted);
            ...
            logger.error("decode_authcookie_error {}", sAuthcookie, e);
            return null;
        }
    }
    ...
    public static AdminSecureAuthCookie valueOf(byte[] data) {
        ByteBuffer authByteBuffer = ByteBuffer.wrap(data);
        authByteBuffer.rewind();
        AdminSecureAuthCookie secureAuthCookie = new AdminSecureAuthCookie();
        secureAuthCookie.uid = authByteBuffer.getLong();
        secureAuthCookie.ctime = authByteBuffer.getInt();
        secureAuthCookie.expire = secureAuthCookie.ctime + 3600;
