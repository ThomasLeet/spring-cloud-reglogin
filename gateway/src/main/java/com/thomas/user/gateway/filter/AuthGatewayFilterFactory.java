package com.thomas.user.gateway.filter;
...
public class AuthGatewayFilterFactory extends AbstractGatewayFilterFactory<AuthGatewayFilterFactory.Config> {
    ...
    public AuthGatewayFilterFactory(Class<Config> configClass) {
        super(configClass);
    }
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            if (cookies != null || !cookies.isEmpty()) {
                ...
                HttpCookie authCookie = cookieMap.get(UserCookie.AUTH);
                if (authCookie != null) {
                    authCookieStr = authCookie.getValue();
                } else {
                    //header
                    HttpHeaders headers = request.getHeaders();
                    String authHeader = headers.getFirst(UserHeaders.FRONT_END_AUTH);
            ...
            try {
                SecureAuthCookie secureAuthCookie = SecureAuthCookie.decodeAuthCookie(authCookieStr);
                if(secureAuthCookie == null || TimeHelper.currentTimeSeconds() > secureAuthCookie.expire){
                    response.setStatusCode(HttpStatus.UNAUTHORIZED);
                    return response.setComplete();
                }
                ServerHttpRequest authRequest = request.mutate().header(UserHeaders.UID, secureAuthCookie.uid + "").build();
